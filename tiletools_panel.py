    ...
    def draw(self, context):
        ...
        if scene.xtd_tools_tiletools_mode == "Bake":
            ...
            if scene.xtd_tools_bake_texture_mode == "EXTENDED":
                ...
                row.use_property_split = True
                row.use_property_decorate = False
                row.label(text="", icon='UV_VERTEXSEL')
                row.prop(scene, "xtd_tools_bake_to_attribute", expand=True)
                ...
                row.prop(scene, "xtd_tools_bake_selectedtoactive", expand=True)
...
class XTD_OT_AppendTileResolution(global_settings.XTDToolsOperator):
    ...
    def execute(self, context):
        ...
        if temp_collection:
            temp_collection_scene = bpy.context.view_layer.layer_collection.children.get("TEMP_LINKED")
...
bpy.types.Scene.xtd_tools_bake_selectedtoactive = bpy.props.EnumProperty(name='SELECTED TO ACTIVE',
...
bpy.types.Scene.xtd_tools_bake_to_attribute = bpy.props.EnumProperty(name='COLOR ATTRIBUTE',
...
# ================== Makeland Operator ==================
class XTD_OT_MAKELAND(global_settings.XTDToolsOperator):
    bl_idname = "xtd_tools.make_land"
    bl_label = "Make land"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def process_object(self, obj):
        working_obj = bpy.data.objects[obj.name]
        bpy.context.view_layer.objects.active = working_obj
        bpy.ops.mesh.customdata_custom_splitnormals_clear()
        obj_project_uuid = obj["project_uuid"]
        obj_uuid_data = global_settings.UUIDManager.parse_project_uuid(obj["project_uuid"])
        objtile_base = obj_uuid_data["uuid_base_tile_name"] 
        
        bpy.ops.xtd_tools.transfermodels(transfer_mode='LINK', source_mode='MASTERFILE', objects='SELECTED', replace_mode='ADD', zoom_level="BP19")
        clear_reports()
        ...
        if temp_collection:
            ...
            if len(linked_objects) < 1:
                print("Nincs linked object")
                return {'CANCELLED'}
            for linked in linked_objects:
                linked.select_set(True)
                bpy.context.view_layer.objects.active = linked
                
            linked_obj = bpy.context.active_object

        working_obj.hide_set(False)
        working_obj.select_set(True)
        bpy.context.view_layer.objects.active = working_obj
        bpy.context.object.modifiers["Subdivision"].levels = 1
        bpy.context.object.modifiers["VertexWeightProximity"].target = linked_obj
        bpy.context.object.modifiers["Shrinkwrap"].target = linked_obj
        bpy.context.object.modifiers["Shrinkwrap.001"].target = linked_obj
        bpy.context.object.modifiers["Shrinkwrap.002"].target = linked_obj
        bpy.context.object.modifiers["Decimate"].ratio = 0.3

        modifiers = [mod.name for mod in working_obj.modifiers]
        for mod in working_obj.modifiers:
